...
- [ ] T005 [P] Configure tooling: ESLint + Prettier + root `tsconfig` references; `vitest.config.ts` with `@cloudflare/vitest-pool-workers` in `apps/api`
...
- [ ] T017 Add auth middleware in `apps/api/src/index.ts` (attach userId; 401 when absent) and mount `routes/auth.ts`
...
### Web shell
- [ ] T027 [P] Implement API client + TanStack Query setup in `apps/web/src/lib/api.ts` + `apps/web/src/lib/query.ts`, and an SSE reader in `apps/web/src/lib/sse.ts`
...
### Tests for US1
- [ ] T029 [P] [US1] Contract test course create + interview SSE in `apps/api/test/us1-interview.test.ts` (vague answer → follow-up; coverage completeness; requirements persisted + validated)
...
- [ ] T034 [US1] Implement `emit_course_requirements` handler (validate → version via T025 → quality gate via T024)
...
- [ ] T038 [US2] Implement `routes/documents.ts` generate endpoint: `POST /courses/:id/documents/:type/generate` (SSE) with ordering guard (main_plan requires completed course_requirements — FR-016)
